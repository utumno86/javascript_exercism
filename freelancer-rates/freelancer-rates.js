...
export function monthRate(ratePerHour, discount) {
  const fullMonthRate = ratePerHour * 8 * 22;
  return Math.ceil(applyDiscount(fullMonthRate, discount));
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  ...
  const discountedRatePerDay = applyDiscount(ratePerDay, discount);
  return Math.floor(budget / discountedRatePerDay);
}
